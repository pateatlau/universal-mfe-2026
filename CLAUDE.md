...
World-class trunk-based development workflow with mandatory E2E gates:

- **PR Checks**: Type checking, linting, unit tests, build verification (`ci.yml`)
- **E2E Gates**: Mandatory Web (Playwright) + Android (Maestro) + iOS (Maestro) E2E before merge (`e2e.yml`)
- **Staging Deployment**: Automatic deployment to staging on push to main (`deploy-staging.yml`)
- **Production Release**: Tag-based production promotion with E2E re-verification (`release.yml`)

**Core Invariant:** `main` is always releasable.

**Key files:**
- `.github/workflows/ci.yml` - Fast CI checks (lint, typecheck, test, build)
- `.github/workflows/e2e.yml` - All-platform E2E tests (merge gate)
- `.github/workflows/deploy-staging.yml` - Staging deployments (Vercel + Firebase)
- `.github/workflows/release.yml` - Production release with E2E re-run
- `docs/CI-CD-IMPLEMENTATION-PLAN.md` - Complete CI/CD architecture documentation
...
- `docs/CI-CD-IMPLEMENTATION-PLAN.md` - CI/CD architecture, workflows, environments, and deployment
- `docs/CI-CD-PREREQUISITES-MANUAL-STEPS.md` - Manual prerequisite steps for CI/CD setup
