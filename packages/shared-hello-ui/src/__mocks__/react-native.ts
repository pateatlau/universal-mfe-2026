 ...
 * that work in a JSDOM environment for unit testing.
 */
...
function createMockComponent(name: string) {
  const Component = React.forwardRef(
    (props: Record<string, unknown>, ref: React.Ref<unknown>) => {
      const { children, testID, accessibilityRole, accessibilityLabel, accessibilityHint, ...rest } = props;
...
export const TextInput = React.forwardRef(
  (props: Record<string, unknown>, ref: React.Ref<unknown>) => {
    const { testID, accessibilityLabel, onChangeText, ...rest } = props;
