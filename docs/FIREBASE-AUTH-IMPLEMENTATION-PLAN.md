...
#### Issue 9: Production Build Missing google-services.json (Firebase Auth Completely Broken)

**Problem:** After deploying v3.5.0 to production via GitHub Actions, ALL Firebase authentication failed on physical Android devices:
- Email/password login failed
- Google Sign-In failed with: "Unable to complete sign-in. Please try again or use a different sign-in method."

Confusingly, local release builds worked perfectly.

**Root Cause (Two Issues):**

1. **Missing google-services.json in CI/CD**: The `google-services.json` file is gitignored (correctly, for security), but the CI/CD workflow wasn't decoding it from secrets. Without this file, the Google Services Gradle plugin wasn't applied:
   ```groovy
   // build.gradle - plugin only applied if file exists
   if (file("google-services.json").exists()) {
       apply plugin: "com.google.gms.google-services"
   }
   ```

2. **Missing Release SHA-1 Fingerprint**: The `google-services.json` only contained the debug keystore SHA-1. Google Sign-In requires the signing certificate's SHA-1 to match what's registered in Google Cloud Console.

**Why Local Release Builds Worked:**
- Local `google-services.json` file existed
- Local release builds fall back to debug keystore when release credentials aren't set:
  ```groovy
  // build.gradle - fallback to debug signing
  if (hasAllCredentials) {
      storeFile file(keystoreFile)
  } else {
      storeFile file('debug.keystore')  // Used locally
  }
  ```

**Fix (Three Parts):**

1. **Add both SHA-1 fingerprints to Firebase Console:**
   - Debug: `5E:8F:16:06:2E:A3:CD:2C:4A:0D:54:78:76:BA:A6:F3:8C:AB:F6:25`
   - Release: `14:42:7D:73:CF:F1:75:F6:F2:9A:EB:37:B4:C2:BA:65:58:8A:8C:CD`

2. **Download updated google-services.json** and update local copy

3. **Update CI/CD workflow** to decode google-services.json from secrets:
   ```yaml
   ...
   - name: Validate required secrets
     run: |
       if [ -z "${{ secrets.GOOGLE_SERVICES_JSON_BASE64 }}" ]; then
         missing_secrets="${missing_secrets}GOOGLE_SERVICES_JSON_BASE64 "
       fi
   ...
   - name: Clean up sensitive files
     if: always()
     run: |
       rm -f packages/mobile-host/android/app/google-services.json
   ```

**GitHub Secret Required:**
```bash
# Create the secret value
base64 -i packages/mobile-host/android/app/google-services.json
# Add as: GOOGLE_SERVICES_JSON_BASE64
```

**Commit:** `13f588f` - fix(ci): add google-services.json decoding for Firebase Auth in production
